...
void EngineMonitor::readSensors() {
  // rpm
  unsigned long readTime = millis();
  if ( lastFlywheelRPMReadTime+config->flywheelRPMReadPeriod > readTime ) {
    readFlywheelRPM();
  }

  // coolant temperature
  if ( lastEngineTemperatureReadTime+config->engineTemperatureReadPeriod > readTime ) {
    readCoolant();
    readOil();
    readFuel();
  ...
  if ( lastVoltageReadTime+config->voltageReadPeriod > readTime ) {
  ...
  if ( lastTemperatureReadTime+config->temperatureReadPeriod > readTime ) {
